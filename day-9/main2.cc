...
struct knot_state
{
    ...
    std::list<std::pair<int, int>> track;
    ...
    void start()
    {
        ...
        //this->track.push_back(this->tail);
    }
    ...
    void print()
    {
        ...
        std::cout << std::endl;
        for (std::pair<int, int> & p : this->track) {
            std::cout << " TRACK: " << p.first << "," << p.second << std::endl;
        }
        std::cout << " size=" << this->tail_set.size();
        std::cout << std::endl;
        if (this->track.size() == 9) {
            std::cout << "T:" << this->track.front().first << ","
                << this->track.front().second << std::endl;
        }
    }
};
        ...
        int count)
{
    ...
    while (count > 0) {
        ...
        if (state.track.size() >= 8) {
            state.track.pop_front();
        }
        if (state.track.size() < 8) {
            state.track.push_back(state.tail);
        }
        if (state.track.size() == 8) {
            state.tail_set.insert(state.track.front());
        }
        state.print();
    }
}
